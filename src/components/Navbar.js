  ...
  return (
    <nav className="flex items-center justify-between flex-wrap bg-teal-500 p-6">
      ...
      <div className={`${open ? "block" : "hidden"} w-full flex-grow sm:flex sm:items-center sm:w-auto`}>
        ...
        <div>
          <Link to="/contribute/" className="inline-block text-sm px-4 py-2 leading-none border rounded text-white border-white hover:border-transparent hover:text-teal-500 hover:bg-white mt-4 sm:mt-0" activeClassName="border-transparent text-teal-500 bg-white">Contribute</Link>
        </div>
      </div>
    </nav>
  )
